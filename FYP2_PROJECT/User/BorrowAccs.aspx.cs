...
{
    public partial class BorrowAccs : System.Web.UI.Page
    {
        ...
        {
            SqlConnection con = new SqlConnection(strcon);
            SqlCommand cmd = new SqlCommand("SELECT * FROM AccessoriesTable WHERE Status = 'AVAILABLE'", con);
            con.Open();
            itemlist.DataSource = cmd.ExecuteReader();
            itemlist.DataTextField = "accsType";
            itemlist.DataValueField = "ID";
            itemlist.DataBind();
            con.Close();
        }
        /*
        protected void ButtonSubmit_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(strcon))
            {
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO RequestsAccs (UserID, AccsID,UserName,UserPhoneNumber,UserEmail, StartDate, EndDate, Status) VALUES (@UserId, @Item,@name,@PhoneNumber,@Email , @StartDate, @EndDate, @Status)", con);
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(strcon))
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        // Add the borrowing information to the database
                        string status = "PENDING";
                        SqlCommand cmdInsert = new SqlCommand("INSERT INTO RequestsAccs ( AccsID, AccsName, UserID, UserName, UserPhoneNumber, UserEmail, StartDate, EndDate, Status) VALUES ( @Item, @ItemName,@UserId,@name, @PhoneNumber, @Email, @StartDate, @EndDate, @Status)", con);
