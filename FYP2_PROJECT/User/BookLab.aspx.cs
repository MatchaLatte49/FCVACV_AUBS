...
namespace FYP2_PROJECT.User
{
    public partial class BookLab : System.Web.UI.Page
    {
        ...
        {
            SqlConnection con = new SqlConnection(strcon);
            SqlCommand cmd = new SqlCommand("SELECT * FROM LABTable WHERE status = 'AVAILABLE' ", con);
            con.Open();
            itemlist.DataSource = cmd.ExecuteReader();
            itemlist.DataTextField = "LabName";
            itemlist.DataValueField = "LID";
            itemlist.DataBind();
            con.Close();
        }
        ...
        private void showImage()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("SELECT ImageName FROM scheduleTable WHERE ImageType = 'LAB'", con);
        ...
        protected void ButtonSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string itemName = itemlist.SelectedItem.Text;
                ...
                using (SqlConnection con = new SqlConnection(strcon))
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        SqlCommand cmdInsert = new SqlCommand("INSERT INTO RequestsLAB (labID, labName, UserID,  UserName, UserPhoneNumber, UserEmail, StartDate, EndDate, Status) VALUES ( @Item,@ItemName, @UserId,@name, @PhoneNumber, @Email, @StartDate, @EndDate, @Status)", con);
        ...
        protected void ButtonSubmit_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(strcon))
            {
                ...
                int userId = int.Parse(CUID);
                string status = "PENDING";
                SqlCommand cmd = new SqlCommand("INSERT INTO RequestsLAB (UserID, labID, UserName,UserPhoneNumber,UserEmail, StartDate, EndDate, Status) VALUES (@UserId, @Item,@name,@PhoneNumber,@Email ,@StartDate, @EndDate, @Status)", con);
