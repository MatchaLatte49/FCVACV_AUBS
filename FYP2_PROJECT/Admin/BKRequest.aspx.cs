...
{
    public partial class BKRequest : System.Web.UI.Page
    {
        ...
        protected void GVbind()
        {
            try
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("SELECT * FROM RequestsBK WHERE Status = 'PENDING' ", con);
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Approve")
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("UPDATE RequestsBK SET Status = @Status, takenVerified = @takenVerified WHERE RequestBKID = @ReqID", con);
                    cmd.Parameters.AddWithValue("@Status", "APPROVED");
                    cmd.Parameters.AddWithValue("@takenVerified", userName);
                    cmd.Parameters.AddWithValue("@ReqID", reqID);
                    cmd.ExecuteNonQuery();
                    cmd = new SqlCommand("UPDATE BKTable SET status = @Status WHERE KID = (SELECT bkID FROM RequestsBK WHERE RequestBKID = @ReqID)", con);
            ...
            else if (e.CommandName == "Reject")
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("UPDATE RequestsBK SET Status = @Status WHERE RequestsBKID = @ReqID", con);
