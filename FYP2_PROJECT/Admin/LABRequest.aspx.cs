...
{
    public partial class LABRequest : System.Web.UI.Page
    {
        ...
        protected void GVbind()
        {
            try
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("SELECT * FROM RequestsLAB WHERE Status = 'PENDING' ", con);
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Approve")
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("UPDATE RequestsLAB SET Status = @Status, takenVerified = @takenVerified WHERE RequestLABID = @ReqID", con);
                    cmd.Parameters.AddWithValue("@Status", "APPROVED");
                    cmd.Parameters.AddWithValue("@ReqID", reqID);
                    cmd.Parameters.AddWithValue("@takenVerified", userName);
                    cmd.ExecuteNonQuery();
                    cmd = new SqlCommand("UPDATE LABTable SET status = @Status WHERE LID = (SELECT labID FROM RequestsLAB WHERE RequestLABID = @ReqID)", con);
            ...
            else if (e.CommandName == "Reject")
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("UPDATE RequestsLAB SET Status = @Status WHERE RequestLABID = @ReqID", con);
